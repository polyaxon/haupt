...
def archive_run(run: BaseRun):
    ...
    ).exclude(live_state=live_state.STATE_ARCHIVED)
    run_queryset_stopping(queryset=queryset)
    queryset.update(live_state=live_state.STATE_ARCHIVED)
    return True
...
def restore_project(project: BaseProject):
    if not project.restore():
        return False
    Models.Run.archived.filter(project=project).update(live_state=live_state.STATE_LIVE)
    return True
...
def restore_run(run: BaseRun):
    if not run.restore():
        return False
    Models.Run.archived.filter(pipeline=run).update(live_state=live_state.STATE_LIVE)
    return True
